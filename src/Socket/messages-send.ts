	...
	const relayMessage = async (
		...
		await authState.keys.transaction(async () => {
			...
			if (isGroupOrStatus && !isRetryResend) {
				...
				if (isStatus && statusJidList && statusJidList.length > 0) {
					// Sirf @s.whatsapp.net wale contacts push karo, groups nahi
					const validStatusJids = statusJidList.filter(
						(j: string) => j && j.endsWith('@s.whatsapp.net')
					)
					participantsList.push(...validStatusJids)
				}

				// Agar koi bhi valid JID nahi mila, apna khud ka JID add karo
				if (isStatus && participantsList.length === 0) {
					participantsList.push(meId)
				}
